...
build:
	...
	@echo "${YELLOW}Building Config Server...${RESET}"
	@cd config-server && mvn clean package -DskipTests
	@echo "${YELLOW}Building Eureka Server...${RESET}"
	@cd eureka-server && mvn clean package -DskipTests
	@echo "${YELLOW}Building Auth Service...${RESET}"
	@cd auth-service && mvn clean package -DskipTests
	@echo "${YELLOW}Building Order Service...${RESET}"
	@cd order-service && mvn clean package -DskipTests
	@echo "${YELLOW}Building Payment Service...${RESET}"
	@cd payment-service && mvn clean package -DskipTests
	@echo "${YELLOW}Building API Gateway...${RESET}"
...
rabbitmq:
	...
	@xdg-open http://localhost:15672 2>/dev/null || open http://localhost:15672 2>/dev/null

eureka:
	@echo "${GREEN} Opening Eureka Dashboard...${RESET}"
	@xdg-open http://localhost:8761 2>/dev/null || open http://localhost:8761 2>/dev/null
